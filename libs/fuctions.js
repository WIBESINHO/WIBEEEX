...
if(!isNumber(user.role)) user.role = '🙊 DUELISTA(A) :v'
...
if (user) { //@skidy89
if (user.level <= 3) {
 user.role = '🙊 APRENDIS(A) :v'
} else if (user.level <= 6) {
  user.role = '🙊 DUELISTA MEDIO(A) III'
} else if (user.level <= 9) {
  user.role = '🙊 DUELISTA(A) II'
} else if (user.level <= 12) {
  user.role = '🙊 DUELISTA(A) I'
